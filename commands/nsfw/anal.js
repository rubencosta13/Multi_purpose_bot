  ...
  async run(message, args) {
    try {
      ...
      if (isCommandEnabled(NSFWSchema, guild.id)) {
        ...
        axios
          .get(`https://nekobot.xyz/api/image?type=${this.name}`) //make it so channel is nsfw idk how 2 make it / i have 2 add the ${this.name} to all of the links????m?????????????????????? mhm
          .then((response) => {
